...
class OrderedBidict(OrderedBidictBase[KT, VT], MutableBidict[KT, VT]):
    ...
    def popitem(self, last=True) -> Tuple[KT, VT]:  # pylint: disable=arguments-differ
        ...
        key: KT = next((reversed if last else iter)(self))  # type: ignore
        val = self._pop(key)
        return key, val
    ...
    @property
    def inverse(self: 'OrderedBidict[KT, VT]') -> 'OrderedBidict[VT, KT]':
        """Overridden only to refine the type hint.

        Ref: https://github.com/python/typing/issues/548
        """
        return cast(OrderedBidict[VT, KT], super().inverse)
