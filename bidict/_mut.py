...
class MutableBidict(BidictBase[KT, VT], MutableBidirectionalMapping[KT, VT]):
    ...
    def update(self, *args: MapOrIterPair, **kw: Mapping[KT, VT]) -> None:  # type: ignore
    ...
    @property
    def inverse(self: 'MutableBidict[KT, VT]') -> 'MutableBidict[VT, KT]':
        """Overridden only to refine the type hint.

        Ref: https://github.com/python/typing/issues/548
        """
        return cast(MutableBidict[VT, KT], super().inverse)
