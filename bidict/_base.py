...
from typing import (
    Any, Callable, ClassVar, Generic, Iterable, Iterator, List, Mapping, MutableMapping, NamedTuple,
...
class BidictBase(BidirectionalMapping[KT, VT]):
    ...
    on_dup: ClassVar[OnDup] = ON_DUP_DEFAULT

    _fwdm_cls: ClassVar[Type[MutableMapping[KT, VT]]] = dict
    _invm_cls: ClassVar[Type[MutableMapping[VT, KT]]] = dict
    ...
    _inv_cls_: ClassVar[Optional[Type['BidictBase[VT, KT]']]] = None  # Type[T_inv] gives "unbound"
    _fwdm: MutableMapping[KT, VT]  # TODO: reuse _fwdm_cls type
    _invm: MutableMapping[VT, KT]  # TODO: reuse _invm_cls type
    _inv: Optional['BidictBase[VT, KT]']  # Optional[T_inv] gives "type variable is unbound"
    _invweak: Optional[ref]  # ReferenceType['T_inv'] gives "type object is not subscriptable"

    #: The callable used by :meth:`__repr__` for printing the contained items.
    _repr_delegate: ClassVar[Callable[[Iterable], Any]] = dict
    ...
    def _init_inv(self) -> None:
        ...
        inv_cls: Type[BidictBase[VT, KT]] = self._inv_cls()
        ...
        self._inv = inv = cast(BidictBase[VT, KT], inv_cls.__new__(inv_cls))
    ...
    @classmethod
    def _inv_cls(cls: Type[T]) -> 'Type[T_inv]':
    ...
    def inverse(self) -> 'BidictBase[VT, KT]':
        ...
        *See also* :attr:`inv`
        ...
        return cast(BidictBase[VT, KT], self._inv)

    @property
    def inv(self) -> 'T_inv':
        """Alias for :attr:`inverse`."""
        # return cast(BidictBase.T_inv, self.inverse)
        return self.inverse  # type: ignore
    ...
    def __repr__(self) -> str:
        ...
        if not self:
            return f'{clsname}()'
        return f'{clsname}({self.__class__._repr_delegate(self.items())!r})'
    ...
    def _put(self, key: KT, val: VT, on_dup: OnDup) -> None:
        ...
        if dedup_result is not _NOOP:
            assert isinstance(dedup_result, _DedupResult)
